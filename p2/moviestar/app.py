...
@app.route('/api/list/trashed', methods=['GET'])
...
def like_movie():
    ...
    receive_movie = request.form['givemovie']
    movie = db.movies.find_one({'title':receive_movie})
    ...
    result = db.movies.update_one({'title':receive_movie}, {'$set': {'likes': new_likes}})
...
def delete_movies():
    # 삭제할 영화의 제목을 받아와 'trashed'값을 true로 바꾼다.
    receive_movie = request.form['movie_give']
    db.movies.update_one({'title':receive_movie},{'$set':{'trashed':True}})
    return jsonify({'result': 'success', 'msg': '삭제되었습니다.'})

# API #6: 영화를 휴지통에서 복구.
@app.route('/api/restore', methods=['POST'])
def restore_movies():
    # 복구할 영화의 제목을 받아와 'trashed'값을 false로 바꾼다.
    receive_movie = request.form['movie_give']
    db.movies.update_one({'title':receive_movie},{'$set':{'trashed':False}})
    return jsonify({'result': 'success', 'msg': '복구되었습니다.'})

# API #6: 영화를 휴지통에서 영구 삭제.
